...
<div class="wa-container" id="wa-container">
  ...
  <a href="https://wa.me/${waNumber}" class="wa-float" target="_blank" title="Chat with us on WhatsApp">
...
function updateFile(filepath) {
    ...
    if (!content.includes('id="wa-container"')) {
        content = content.replace('</body>', globalElements + '\\n</body>');
    }
    ...
    // specifically handle the generic product image placeholder in products.html
    if (content.includes('<span class="card-image-label">[')) {
        content = content.replaceAll('<span class="card-image-label">[', '<img src="/images/interior_hero_1.jpg" alt="Product" style="width:100%;height:100%;object-fit:cover; position:absolute; top:0; left:0; z-index:0;"> <span style="display:none" class="card-image-label">[');
    }
    
    if (content.includes('<div class="hero-image-placeholder">')) {
         content = content.replaceAll('<div class="hero-image-placeholder">', '<div class="hero-image-placeholder" style="padding:0;">');
    }

    if (filepath.includes('products.html')) {
        const filterScript = `
<script>
  document.addEventListener('DOMContentLoaded', () => {
    const checkboxes = document.querySelectorAll('.filter-checkbox');
    
    // WhatsApp Know More button injection
    ...
    function applyFilters() {
        const activeBrand = document.querySelector('.brand-panel.active');
        if (!activeBrand) return;
        const cards = activeBrand.querySelectorAll('.product-card');
    ...
    // Override switchBrand if exists to apply filters on tab change
    if (typeof window.switchBrand !== 'undefined') {
        const originalSwitch = window.switchBrand;
        window.switchBrand = function(brand) {
            originalSwitch(brand);
            setTimeout(applyFilters, 50);
        };
    } else {
        window.switchBrand = function(brand) {
            document.querySelectorAll('.brand-tab').forEach(t => t.classList.remove('active-vegam', 'active-cycle'));
            document.querySelectorAll('.brand-panel').forEach(p => p.classList.remove('active'));
            
            const activeTab = event.currentTarget || document.querySelector(\`.brand-tab:contains('\${brand}')\`);
            if (activeTab) activeTab.classList.add(\`active-\${brand}\`);
...
`;
        if (!content.includes('// WhatsApp Know More button injection')) {
...
`;
        if (!content.includes('function handleSubmit()')) {
...
`;
        if (!content.includes('animate-on-scroll')) {
            content = content.replace('</head>', animationCss + '\\n</head>');
            content = content.replaceAll('class="brand-card ', 'class="brand-card animate-on-scroll ');
            content = content.replaceAll('class="dealer-inner"', 'class="dealer-inner animate-on-scroll"');
            content = content.replaceAll('class="insp-grid"', 'class="insp-grid animate-on-scroll"');
        }
    }
...
<script>
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.problem-card, .problem-title, .problem-text, .timeline-item').forEach(el => {
